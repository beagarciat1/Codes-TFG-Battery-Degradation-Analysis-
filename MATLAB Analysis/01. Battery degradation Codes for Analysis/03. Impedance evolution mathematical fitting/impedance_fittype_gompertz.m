...
% ==================== STARTPOINTS PERSONALIZADOS (Gompertz) ====================
startPoints = {
 [ 1e-8,   5, 0.008, 8.7e-8],  ...   % L0_H     → ruido, amplitud pequeña
[ 0.015,  5, 0.008, 0.150],   ...   % R0_Ohm   → sube suave
[ 0.005,  4, 0.01,  0.057],   ...   % R1_Ohm   → no monótono, ver nota
[-0.0004, 5, 0.008, 0.0017],  ...   % C1_F     → ligera bajada
[-0.062,  5, 0.008, 0.170],   ...   % R2_Ohm   → bajada marcada
[ 0.008,  6, 0.008, 0.013]    ...   % C2_F     → subida clara;
}
outputFolder = 'Fits_Gompertz';
if ~exist(outputFolder, 'dir'), mkdir(outputFolder); end

fprintf('=== AJUSTE GOMPERTZ (y = a*exp(-b*exp(-c*x)) + d) ===\n\n');
...
for i = 1:length(parametros)
    ...
    ft = fittype('a*exp(-b*exp(-c*x)) + d', 'independent','x');
    ...
    fprintf('Ecuación: y = %.6e * exp(-%.6e * exp(-%.6e * x)) + %.6e\n', ...
    ...
    plot(x_fino, y_fino, 'r-', 'LineWidth',2.8, 'DisplayName','Gompertz');
    title(['Ajuste Gompertz - ' nombres{i}], 'FontSize',15);
    xlabel('Ciclo', 'FontSize',13); ylabel(nombres{i}, 'FontSize',13);
    grid on; legend('Location','best');
    
    saveas(gcf, fullfile(outputFolder, ['Gompertz_' var_name '.png']));
end

disp('Ajuste Gompertz terminado.');