function tabla_parametros = plot_fit_nyquist_PSO(rutaCarpeta)
    ...
    archivos = dir( ...
        fullfile(rutaCarpeta, '*.xlsx'));

    if isempty(archivos)
        error('No se encontraron archivos .xlsx en la carpeta: %s', rutaCarpeta);
    end

    ciclos_excluir = [7];   % Puedes poner [7, 20, 30] si quieres excluir varios

    colores = lines(length(archivos));

    figure('Name', 'Diagramas de Nyquist con ajuste PSO', 'Color', 'w');
    hold on;
    grid on;
    axis equal;
    xlabel('Z_{real} (\Omega)');
    ylabel('-Z_{imaginaria} (\Omega)');
    title('Diagramas de Nyquist con ajuste (PSO)');
    ...
    for k = 1:length(archivos)
        ...
        try
            ...
            z_model = @(x, w) x(1) + x(6)*1i*w + ...
             ...
             % Función objetivo: suma de cuadrados de residuos
            objective = @(x) sum( abs( z_model(x, w) - Z_exp ).^2 );

            % Límites (mismos que antes)
            ...
            ub = [R0_est*1.2, deltaR, 1, deltaR, 1, 1];   % Un poco más amplio para PSO
            ...
            options = optimoptions('particleswarm', ...
                ...
                'UseParallel', false);   % Pon true si tienes Parallel Computing Toolbox

            % Ejecutar PSO
            [x_opt, fval] = particleswarm(objective, 6, lb, ub, options);
            ...
            leyenda{end+1} = sprintf('%s fit (PSO)', nombreArchivo);
    ...
    %writetable(tabla_parametros, fullfile(rutaCarpeta, 'parametros_ajuste_PSO.xlsx'));
% ==================== MEDIAS AÑADIDAS ====================
...
function hacer_grafica_parametro(ciclos, valores, nombreParametro, etiquetaY)
    ...
    figure('Name', ['Evolución de ', nombreParametro, ' (PSO)'], 'Color', 'w');
